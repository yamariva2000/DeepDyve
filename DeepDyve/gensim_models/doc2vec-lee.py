...
np.random.seed(1)
choice=np.random.choice(xrange(30),size=10,replace=False)
...
from gensim import utils
random_choices=np.random.choice(xrange(200000),size=1000,replace=False)
sql='select permdld, body from docs where id2 in {}'.format(tuple(choice))
...
def read_corpus(tokens_only=False):  # generates document tokens for the dictionary
